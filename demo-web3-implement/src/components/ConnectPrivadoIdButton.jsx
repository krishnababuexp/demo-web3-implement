...
function ConnectPrivadoIdButton() {
    const [account, setAccount] = useState(null);
      ...
      const handleBackendLogin = async () => {
        const response = await axios.get(`${process.env.VITE_VERIFIER_URL}/api/auth/did/login/init/`);
        console.log("verifier response",response.data);
        const hostUrl = process.env.VITE_VERIFIER_URL;
        const sessionId = 1;
        ...
        const url = `${process.env.VITE_VERIFIER_URL}?redirect_ext=https://metamask.dyneum.io`;
